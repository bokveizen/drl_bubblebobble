...
def search(index, level):
    ...
    obs = env.reset()
    max_enemies = [0, 3, 4, 4, 6, 4, 4, 4, 4, 7, 7, 7, 6, 7, 7, 7, 7, 7, 6, 7, 5, 7, 4, 5, 7, 6, 7, 7, 7, 6, 7, 7, 7, 6,
    ...
    display = 0
    action_index_list_saved = []
    action_index_list = []
    if action_index_list_saved:
        action_index_list = action_index_list_saved[:]
        ...
        if not action_buffer:
            if action_index is None:  # first step
                ...
                if new_lives < rem_lives:  # lives down, reset
                    ...
                    # if action_index_list_saved:
                    #     print('action_index_list_saved', len(action_index_list_saved), action_index_list_saved)
                    ...
                    if not action_index_list_saved and new_enemies <= max_enemies[new_level] // 2:
                        action_index_list_saved = action_done[:-20]
                    if action_index_list_saved:
                        action_index_list = action_index_list_saved[:]
                    if not action_index_list:
                        action_index = None
                    ...
                    zero_enemy_count = 0
                    continue
                elif new_level > rem_level or (rem_level == 99 and zero_enemy_count >= 10):  # level up, save
